...
def write_score(input_score):
    bk_img = cv2.imread(template_path + "temp_score.jpg")
    font_path = template_path + "MSJH.ttf"
    font = ImageFont.truetype(font_path, 600)
    img_pil = Image.fromarray(bk_img)
    draw = ImageDraw.Draw(img_pil)
    draw.text((650, 360), str(input_score), font=font, fill=(255, 255, 255))
    bk_img = np.array(img_pil)
    cv2.imwrite(template_path + "temp_score.jpg", bk_img)
...
def write_result(input_list):
    bk_img = cv2.imread(template_path + "grid_zh.png")
    font_path = template_path + "MSJH.ttf"
    ...
    for temp_input in input_list:
        ...
        if temp_input['level'] == '低' and temp_input['cost'] == 'High':
            ...
            count_lh += 1
    bk_img = np.array(img_pil)
    cv2.imwrite(template_path + "temp_grid.png", bk_img)


def write_result_en(input_list):
    bk_img = cv2.imread(template_path + "grid_en.png")
    font_path = template_path + "MSJH.ttf"
    font = ImageFont.truetype(font_path, 20)
    img_pil = Image.fromarray(bk_img)
    draw = ImageDraw.Draw(img_pil)

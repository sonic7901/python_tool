...
def add_issue(input_docx,
    ...
    input_document = Document(input_docx)
    if input_en:
        document_table = Document(template_path + 'template_table_issue_en.docx')
    else:
        document_table = Document(template_path + 'template_table_issue_zh.docx')
...
def transfer_report(report_data_list):
        ...
        try:
            ...
            for site in report_data['site']:
                ...
                for alert in site['alerts']:
                    ...
                    # test en to zh
                    try:
                        df = pandas.read_excel(template_path + "zap_report.xlsx")
                        nmp = df.values
                        for n in nmp:
    ...
    utils.custom_image.write_result(issue_list)
    # update image
    path = template_path + "template_vas_zh.docx"
    doc = DocxTemplate(path)
    utils.custom_chart.zap_score(count_low, count_medium, count_high)
    if count_low == 0 and count_medium == 0 and count_high == 0:
        shutil.copyfile(template_path + 'none_issue.jpg', template_path + 'temp_distribution.jpg')
    else:
        utils.custom_chart.zap_pie(count_low, count_medium, count_high)
    ...
    replacements = {
        ...
        'image_score': InlineImage(doc, template_path + 'temp_score.jpg', width=Mm(90)),
        'image_dis': InlineImage(doc, template_path + 'temp_distribution.jpg', width=Mm(90)),
        'image_9': InlineImage(doc, template_path + 'temp_grid.png', width=Mm(180)),
    }
    doc.render(replacements)  # 渲染替换
    doc.save(input_fn)  # 保存

    # clear image
    os.remove(template_path + 'temp_score.jpg')
    os.remove(template_path + 'temp_distribution.jpg')
    os.remove(template_path + 'temp_grid.png')
    ...
    for report_data in report_data_list:
        ...
        if default_screenshot_url == '':
            ...
            read_screenshot(default_screenshot_url, template_path + report_data['target'] + '.png')
        add_screenshot(input_fn, template_path + report_data['target'] + '.png',
                       str(target_count) + '.' + report_data['target'] + '網站進入點:')

        os.remove(template_path + report_data['target'] + '.png')
...
def transfer_report_en(report_data_list):
    ...
    utils.custom_image.write_result_en(issue_list)

    # update image
    path = template_path + "template_vas_en.docx"
    doc = DocxTemplate(path)
    utils.custom_chart.zap_score(count_low, count_medium, count_high)
    if count_low == 0 and count_medium == 0 and count_high == 0:
        shutil.copyfile(template_path + 'none_issue.jpg', template_path + 'temp_distribution.jpg')
    else:
        utils.custom_chart.zap_pie_en(count_low, count_medium, count_high)
...
if __name__ == '__main__':
    ...
    set_company('example')
    # normal example
    main_json_1 = read_json('../models/example.json')
